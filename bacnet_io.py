...
class BacnetDev(BacnetBase):
  ...
  def _read(self, prop, array_index = -1):
    return bacnet.read_prop(self.h_dev, bacnet.OBJECT_BINARY_VALUE, self.h_dev['device_id'],prop,array_index)
...
class BacnetIO:
  ...
  def __init__(self, db_file, nic = None, port = None):
    filename = os.path.join(os.path.dirname(__file__), db_file)
    if not os.path.exists(filename):
      raise Exception("Database file not found: " + filename)
  
    self.dev_list = pickle.load(open(filename, 'rb')) 
  ...
  def write_point(self, obj_name, otype, value, priority = 16):
    if type(obj_name) == tuple:
      o = self.find(obj_name[0])
    else:
      o = self.find(obj_name)
    if o == None:
      raise Exception('Point %s not found.' % obj_name)
    return o.write_point(otype, value, priority)
